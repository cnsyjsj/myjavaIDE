...
bool matchconst1(int n,char s[65536],int m=0){
	...
	switch(c){
		...
		default:	read2(C,&tb);
		if(!matchconst1(tb,s))	return 0;pi++;
...
us fndc1(cc c[65536],int m=0){//[-add][deep]
	...
	if(ti==0&&m==0){
		...
		switch(t){
			...
			out8(stdout,copos[fcnt[_const_pool]]);
			addc(fcnt[_const_pool],t);printf("\n");
			out8(stdout,copos[fcnt[_const_pool]]);printf("\n");
			pa2(inpnam,copos[fcnt[_const_pool]-1]+1,ti);
